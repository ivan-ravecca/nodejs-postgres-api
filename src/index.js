...
app.post('/events', [
  ...
  try {
    ...
    const result = await pool.query('INSERT INTO events (eventId, notes, event, timestamp) VALUES ($1, $2, $3, $4) RETURNING *', [eventId, notes, event, timestamp]);
...
app.put('/events/:id', [
  ...
  try {
    ...
    const result = await pool.query('UPDATE events SET notes = $1, timestamp=$2, event=$3 WHERE eventid = $4 RETURNING *', [notes, timestamp, event, id]);
